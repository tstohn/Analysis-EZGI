...
for THREAD in "${THREADS[@]}"; do
    LOG_ESGI_RNA="./SIGNALseq_Analysis/output/RUNTIME_TEST/LOG_FILES/ESGI_RNA_${THREAD}_LOG.txt"
    LOG_ZUMI="./SIGNALseq_Analysis/output/RUNTIME_TEST/LOG_FILES/ZUMI_${THREAD}_LOG.txt"
    rm -f $LOG_ESGI_RNA
    rm -f $LOG_ZUMI
done
...
for ((i=1; i<=REPEATS; i++)); do
  ...
  for THREAD in "${THREADS[@]}"; do
    ...
    /usr/bin/time -v /DATA/t.stohn/analyses_ezgi/SIGNALseq_Analysis/zUMIs/zUMIs.sh \
                  -c \
                  -y "/DATA/t.stohn/analyses_ezgi/SIGNALseq_Analysis/background_data/zUMI_files/zUMI_params_SIGNALseq_${THREAD}threads.yaml" > /dev/null 2>> $LOG_ZUMI
