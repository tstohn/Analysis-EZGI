...
for THREAD in "${THREADS[@]}"; do
    LOG_ESGI_PROT="./SIGNALseq_Analysis/output/RUNTIME_TEST/LOG_FILES/ESGI_PROT_SMALL_${THREAD}_LOG.txt"
    LOG_KITE="./SIGNALseq_Analysis/output/RUNTIME_TEST/LOG_FILES/KITE_SMALL_${THREAD}_LOG.txt"
    rm -f $LOG_ESGI_PROT
    rm -f $LOG_KITE
done
...
for ((i=1; i<=REPEATS; i++)); do
  ...
  for THREAD in "${THREADS[@]}"; do
    ...
    #before adding readname the structure was: -c 1,3,5 -x 0 -u 6
